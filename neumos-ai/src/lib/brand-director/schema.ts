...
export const BrandPlanSchema = z.object({
  ...
  audiences: z.array(z.string().min(1)).min(1).max(3),
  customerIntent: z.string().min(1),
  moodKeywords: z.array(z.string().min(1)).min(3).max(6),
  ...
  photoAssignments: z.array(PhotoAssignmentSchema),
...
export const BRAND_PLAN_JSON_SCHEMA = {
  ...
  schema: {
    ...
    properties: {
      ...
      moodKeywords: { type: "array", items: { type: "string" }, minItems: 3, maxItems: 6 },
