...
export function renderWebsitePreviewHtml(brief: StoreBrief, contents: GeneratedWebsiteContents): string {
  ...
  const contactMethodsHtml = contents.contactMethods
    .map((m) =>
      m.href
        ? `<a class="pill" href="${escapeHtml(m.href)}">${escapeHtml(m.label)}</a>`
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .pill { display: inline-block; margin: 4px; padding: 6px 14px; border-radius: 999px; border: 1px solid ${theme.cardBorder}; font-size: 12px; color: ${theme.accent}; text-decoration: none; }
...
<body>
  ...
  <div class="cta">
    ...
    <a class="btn" href="${escapeHtml(contents.cta.href)}">${escapeHtml(contents.cta.buttonLabel)}</a>
